...
export class UserController {
    ...
    static async initiateGoogleOAuth(
        ...
        try {
            const response = await http.get(
                `${this.basePath}/authentication/oauth2/google`,
                { params: { redirectUri } },
            );
     ...
     */
    static async handleGoogleOAuthCallback(
     ...
     */
    static async initiateGitHubOAuth(
        redirectUri: string,
    ): Promise<ApiResponse<{ authUrl: string }>> {
        try {
            const response = await http.get(
     ...
     */
    static async handleGitHubOAuthCallback(
        request: OAuthCallbackRequest,
    ): Promise<ApiResponse<AuthenticationResponse>> {
        try {
            const response: AxiosResponse<AuthenticationResponse> =
    ...
    > {
        try {
            const response: AxiosResponse<AuthenticationResponse> =
                await http.get(`${this.basePath}/users/me`);
            return { success: true, data: response.data };
        } catch (error) {
            return this.handleError(error);
        }
    ...
    static async refreshToken(
        ...
        try {
            const response: AxiosResponse<AuthenticationResponse> =
                await http.post(`${this.basePath}/authentication/refresh`, {
                    refreshToken,
                });
            return { success: true, data: response.data };
        } catch (error) {
    ...
    > {
        try {
            const response = await http.post(
                `${this.basePath}/authentication/validate`,
            );
            return { success: true, data: response.data };
        } catch (error) {
            return this.handleError(error);
